package com.ztb.nankerlibrary.control.xexpandablelistview;
...
import com.ztb.nankerlibrary.R;
...
public class XExpandableListView extends ExpandableListView implements OnScrollListener {
	...
	/**
	 * default :it only can load more when the total count >=7 at last
	 * 	you can call the method {@link #()}
	 */
	private static int VISIBLECOUNT = 7;
	private int viewHeight = 0;		//the height of XExpandableListView
	private int windowHeight = 0;	//the height of window
	/**
	 * default: 120px
	 * you can set the itemHeight that you need,
	 * but advice set the itemHeight 120px at last,
	 * of course you can set the itemHeight that you think the better and more modest value
	 */
	private final static int itemHeight = 120;
	...
	private void initWithContext(Context context) {
		...
		windowHeight = metrics.heightPixels; // 将获取的宽度存入到全局变量中
	}
	...
	public void setPullLoadEnable(boolean enable) {
		...
		Log.i(TAG, "mFooterView.isShown()==+++==>" + mFooterView.isShown());
	}
	...
	private void updateHeaderHeight(float delta) {
		...
		/** the state is not refreshing, and then update the arrow*/
...
//		setSelection(mTotalItemCount - 1); // scroll to bottom
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getFirstVisiblePosition() == 0 && (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
				...
				if (VISIBLECOUNT*itemHeight> windowHeight && getLastVisiblePosition() == mTotalItemCount - 1
					&& (mFooterView.getBottomMargin() > windowHeight || deltaY < -mFooterView.getBottomMargin()/2)) {
	...
	public void setExpandableListViewListener(IXExpandableListViewListener l) {
		mListViewListener = l;
	}
	...
	/**
	 * if it is the state that is loading
	 * @return
	 */
	...
	/**
	 * set a more modest visibleCount which you think
	 * @param visibleCount
	 */
	public void setVisibleCount(int visibleCount){
		if (visibleCount*itemHeight<windowHeight) {
			return;
		}
